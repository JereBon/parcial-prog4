...
from ..models import Producto, Categoria, Ingrediente, ProductoCategoria, ProductoIngrediente
...
class ProductoService:
    ...
    def get_all(self, skip: int = 0, limit: int = 100):
        query = select(Producto).where(Producto.is_active == True).options(selectinload(Producto.categorias), selectinload(Producto.ingredientes)).offset(skip).limit(limit)
        return self.session.exec(query).unique().all()

    def get_by_id(self, producto_id: int):
        query = select(Producto).where(Producto.id == producto_id).options(selectinload(Producto.categorias), selectinload(Producto.ingredientes))
    ...
    def create(self, producto_in: ProductoCreate):
        ...
        for cat_id in producto_in.categorias_ids:
            ...
            if cat:
                self.session.add(ProductoCategoria(producto_id=producto.id, categoria_id=cat.id))
        
        for ing_id in producto_in.ingredientes_ids:
            ing = self.session.get(Ingrediente, ing_id)
            if ing:
                self.session.add(ProductoIngrediente(producto_id=producto.id, ingrediente_id=ing.id))
    ...
    def update(self, producto_id: int, producto_in: ProductoCreate):
        ...
        for pc in self.session.exec(select(ProductoCategoria).where(ProductoCategoria.producto_id == producto.id)).all():
            self.session.delete(pc)
        for pi in self.session.exec(select(ProductoIngrediente).where(ProductoIngrediente.producto_id == producto.id)).all():
